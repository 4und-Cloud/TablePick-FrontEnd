...
export default function Modal( {onClose, children, footer, close } : ModalProps) {    
    return ReactDOM.createPortal(
        <div className='fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50'>
            <div className='bg-white p-2 rounded-20 shadow-xl relative w-[540px] h-[540px] border-2 border-main flex flex-col justify-between'>
                {close && (<button className='absolute right-4 font-bold text-[20px] text-main' onClick={onClose}>X</button>)}
                {children}
                {footer && (
