    ...
    const handleLogin = async (provider: string) => {
        try {
            ...
            window.addEventListener(
                ...
                async (event) => {
                    // 로그인 성공 메시지를 받았을 때
                    if (event.data && event.data.type === "LOGIN_SUCCESS" && event.data.userId) {
        ...
        <>
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                <div className="bg-login w-[800px] h-[600px] shadow-2xl flex relative">
                    <button onClick={onClose} className="absolute text-black top-4 right-4 text-2xl z-10">
                        ×
                    </button>
                    {/* 왼쪽 영역 */}
                    ...
                    <div
                        className="absolute bg-black bg-opacity-10 left-1/2 w-px h-full transform -translate-x-1/2"></div>
                    {/* 오른쪽 영역 */}
