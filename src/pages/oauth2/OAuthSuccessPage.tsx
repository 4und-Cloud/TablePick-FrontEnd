  ...
  useEffect(() => {
    async function fetchUserInfo() {
      try {
        ...
        const hasCompletedAdditionalnfo = localStorage.getItem(`hasCompletedAdditionalInfo_${userData.id}`);

        // isNewUser는 최근 생성되었고 AND 아직 추가 정보 모달을 완료하지 않은 경우
        // false, 신규 유저가 아닌 경우 
        const shouldShowAdditionalInfoModal = isUserRecentlyCreated && !hasCompletedAdditionalnfo;
        ...
        console.log("OAuth 성공 페이지 - 정규화된 사용자 데이터:", normalizedUser);
        // 로그인 처리
  ...
  if (error) {
    return (
      <div className="mt-[80px] text-center">
        ...
        <button onClick={() => navigate("/")} className="mt-4 px-4 py-2 bg-main text-white rounded-md">
          홈으로 돌아가기
        </button>
      </div>
    )
